...
xKiro API credentials and Google Flow cookies remain protected by Windows DPAPI. Desktop sessions generate a random per-process session token. Mutating API requests (`POST`, `PUT`, `PATCH`, `DELETE`) require that token in `X-Flow-Studio-Session`. The token is passed to the frontend only through the initial URL fragment and removed from browser history immediately after startup.
...
`requirements.lock.txt` contains the validated Windows/Python 3.12 constraint set. The vendored `flow_cli-0.6.0-py3-none-any.whl` is additionally pinned by SHA-256 in `vendor/SHA256SUMS.txt` because it is not available for PyPI vulnerability lookup.

## External provider risk

Google Flow remains an external browser/API dependency outside this application's control. Production support should treat Flow UI/API changes as a provider outage risk. The application has checkpoint/recovery and compatibility handling, but no local release can guarantee an upstream Google interface remains stable.
